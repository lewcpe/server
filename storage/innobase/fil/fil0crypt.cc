	...
	byte*		dst_frame)	/*!< in: outbut buffer */
{
	...
	if (tmp) {
		...
		bool corrupted1 = buf_page_is_corrupted(true, src_frame, page_size, fsp_is_checksum_disabled(space));
		...
		bool different = memcmp(src_frame, tmp_mem, page_size.physical());
